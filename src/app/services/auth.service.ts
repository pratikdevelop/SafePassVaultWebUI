...
export class AuthService {
  ...
  startWebAuthnAuthentication(challange: string, credential: any, email: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/webauthn/complete-authenticate`, {
      credential, challange, email
    }).pipe(map(
      (response) => {
        return response
      },
    ...
    return new Observable<string | null>((observer) => {
      this.afMessaging.requestToken.subscribe(
        (token) => {
          ...
          observer.next(token);  // Emit the token
          observer.complete();  // Complete the Observable
        },
        (error) => {
          console.error('Unable to get permission:', error);
          observer.error(null);  // Emit an error if permission fails
        }
      );
    });
  }
